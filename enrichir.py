...
## on elimine les doubles avec le dict
list_trie = dict.fromkeys(sorted(dic))

## on genere le fichier info3
Min = ord('A')
Max = ord('Z')
...
for i in range(Min, Max + 1):
    ...
    for c in temp:
        if (c[0].lower() == current_char.lower()):
            cpt_char = cpt_char + 1
    info3.write("le nombre de med commencant par la lettre " + current_char +
                " est : " + str(cpt_char) + "\n")
    sum = sum + cpt_char
info3.write("le nombre total de med est : " + str(sum) + "\n")
info3.close()
...
for token in list_enrich:
    if token.lower().startswith(first_car) and token.lower(
        ...
        if token != "puis" and not token.startswith(
            ...
            #Si notre script rencontre un mot qui ne commencent pas le terme courant , alors il affiche le nombre de medicament commencant par la dite lettre
            test.write(
                "Le nombre de médicaments provenant de l'enrichissement qui commencent par la lettre "
                + first_car + " sont: " + str(cpt) + "\n")
            first_car = token[0]
            cpt = 1
# notre boucle ne couvre pas le boucle donc on le fait manuelment
test.write(
    "Le nombre de médicaments provenant de l'enrichissement qui commencent par la lettre "
    + first_car + " sont: " + str(cpt) + "\n")
test.write(
